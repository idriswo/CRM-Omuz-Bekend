...
## Gmail SMTP → Gmail API (2026-07-28)

Brevo (ниг. поён) бекор карда шуд: сабтиноми он тасдиқи рақами телефонро талаб
мекунад ва SMS-и код нарасид. Ба ҷои он ҳамон почтаи Gmail, вале аз болои
**Gmail API (HTTPS, порти 443)** — портҳои SMTP-и Render ба ин роҳ халал
намерасонанд.

- `mailProvider()` акнун се ҳолат дорад:
  - **`gmail-api`** — агар `GOOGLE_CLIENT_ID` + `GOOGLE_CLIENT_SECRET` + `GOOGLE_REFRESH_TOKEN` бошанд
  - **`gmail-smtp`** — `GMAIL_USER` + `GMAIL_APP_PASSWORD`, танҳо барои dev
  - **`stub`** — ҳеҷ калид нест, паём ба лог меравад
- `scripts/gmail-oauth.ts` — скрипти якдафъаина барои гирифтани `refresh_token`
- Scope танҳо `gmail.send` — хондани почта иҷозат дода намешавад
- Паём ҳамчун MIME-и хом сохта мешавад (`multipart/alternative`), сарлавҳаи
  тоҷикӣ тибқи RFC 2047 кодкунӣ мешавад
- `sendEmail()`, шаблонҳо ва controller-ҳо боз ҳам **бетағйир монданд**
- Бартарӣ: домен, пардохт ва провайдери сеюм лозим нест. Лимит ~500 email/рӯз
- `googleapis` аллакай дар вобастагиҳо буд — бастаи нав илова нашуд

## Gmail SMTP → Brevo (2026-07-27, бекор шуд)
